...
class AnimationPlayer:
    ...
    def _load_sequence_from_animation_data(self, animation_data, frame_images):
        """Load animation sequence from animation data list.

        Args:
            animation_data: List of dicts with 'frame' and 'duration' keys
            frame_images: Dict mapping frame numbers to PhotoImage objects

        Returns:
            List of (frame_num, duration_ms, image) tuples
        """
        sequence = []
        for frame_data in animation_data:
            frame_num = frame_data.get("frame")
            duration_ticks = frame_data.get("duration", 0)

            image = frame_images.get(frame_num)
            if image is not None and duration_ticks > 0:
                duration_ms = int(duration_ticks * MS_PER_TICK)
                sequence.append((frame_num, duration_ms, image))

        return sequence

    def _update_frame_selector(self, reset_index=True):
        """Update frame spinbox with current sequence frames.

        Args:
            reset_index: If True, reset to first frame. If False, preserve playing state.
        """
        if self.current_sequence:
            ...
            if reset_index or not self.is_playing:
                self.current_frame_index = 0
    ...
    def _create_playback_controls(self, parent):
        """Create common playback controls (Loop checkbox, Play button, Toggle BG)."""
    ...
    def _create_frame_selector(self, parent, width=8):
        """Create frame selector spinbox (packed on right side)."""
...
class InfoDialog:
    ...
    def _build_ui(self):
        ...
        desc_label = tk.Label(
            content_frame,
            text="A tool for Explorers of Sky that converts\nframes into wan sprite and wan sprite back to frames",
            font=("Arial", 10),
            justify=tk.CENTER,
            wraplength=340,
        )
...
class AnimationViewer(AnimationPlayer):
    ...
    def _load_animation(self, anim_index):
        ...
        self.current_sequence = self._load_sequence_from_animation_data(
            animation, self.frame_number_to_image
        )
        self._frame_num_to_index = {
            frame_num: idx
            for idx, (frame_num, _, _) in enumerate(self.current_sequence)
        }
...
class AnimationEditorDialog(AnimationPlayer):
    ...
    def _create_preview_pane(self, parent):
        ...
        self._create_playback_controls(control_frame)
        self._create_frame_selector(control_frame, width=5)
    ...
    def _update_preview(self):
        # Build animation data from current frame entries
        animation_data = []
        ...
        self.current_sequence = self._load_sequence_from_animation_data(
            animation_data, self.frame_images
        )

        # Use inherited method, but preserve playing state
        self._update_frame_selector(reset_index=False)
    ...
    def _save_and_close(self):
        ...
        for frame_var, duration_var, _ in self.frame_entries:
            try:
                ...
                messagebox.showerror(
                    "Invalid Input",
                    "Please ensure all frame and duration fields are filled in correctly.",
                    parent=self.dialog,
                )
                return
        ...
        if self.available_frames:
            ...
            if invalid:
                messagebox.showerror(
                    "Invalid Frames",
                    f"The following frames are not available: {invalid}\n\nPlease use only valid frame numbers.",
                    parent=self.dialog,
                )
                return
...
class WanimationStudioGUI:
    ...
    def create_frames_generator_tab(self, parent):
        ...
        base_sprite_frame.pack(fill=tk.X, pady=(10, 0))
        ...
        overlap_frame.pack(fill=tk.X, pady=(10, 0))

        self.avoid_overlap = tk.StringVar(value="none")
        ...
        self.generate_frames_btn.pack(fill=tk.X, pady=(10, 0))
    ...
    def create_wan_io_tab(self, parent):
        ...
        generate_frame.pack(fill=tk.X, pady=(15, 0))
        ...
        extract_frame.pack(fill=tk.X, pady=(10, 0))
    ...
    def create_basic_settings(self, parent):
        ...
        folder_frame.grid(row=row, column=1, sticky=tk.EW, padx=(5, 0))
        ...
        category_combo.grid(row=row, column=1, sticky=tk.EW, pady=5, padx=(5, 0))
        ...
        self.flags_label = ttk.Label(parent, text="Flags:", font=("Arial", 9, "bold"))
        self.flags_label.grid(row=row, column=0, sticky=tk.W, pady=5)

        self.category_props_frame = ttk.Frame(parent)
        self.category_props_frame.grid(
            row=row, column=1, sticky="ew", pady=5, padx=(5, 0)
        )
        ...
        self.category_checkboxes["used_base_palette"].pack(side=tk.LEFT, padx=(5, 0))
    ...
    def _update_category_props_visibility(self, category=None):
        ...
        # Show/hide the entire flags row (label + checkboxes frame)
        if visible_checkboxes:
            self.flags_label.grid()
            self.category_props_frame.grid()
        else:
            self.flags_label.grid_remove()
    ...
    def create_animation_settings(self, parent):
        ...
        list_frame.grid(row=0, column=0, sticky="nsew", pady=(10, 18), padx=10)
        ...
        btn_frame.grid(row=0, column=1, sticky="n", pady=(10, 18), padx=(0, 10))
    ...
    def add_animation_sequence(self):
        if not self.sg_available_frames:
            messagebox.showwarning(
                "No Folder Selected",
                "Please select a folder with valid images before adding animations.",
            )
            return
    ...
    def edit_animation_sequence(self):
        ...
        if not selection:
            messagebox.showwarning(
                "No Selection",
                "Please select an animation sequence from the list to edit.",
            )
            return
    ...
    def delete_frame_or_sequence(self):
        ...
        if not selection:
            messagebox.showwarning(
                "No Selection",
                "Please select an animation or frame from the list to delete.",
            )
            return
        ...
        if is_frame:
            ...
            if is_last_sequence and is_last_frame_in_sequence:
                messagebox.showwarning(
                    "Cannot Delete",
                    "At least one animation sequence with one frame is required.",
                )
                return
            ...
            if messagebox.askyesno(
                "Confirm Delete",
                f"Are you sure you want to delete frame {frame_no} from Animation {group_idx + 1}?",
            ):
            ...
            if is_last_sequence:
                messagebox.showwarning(
                    "Cannot Delete",
                    "At least one animation sequence is required.",
                )
                return
            ...
            if messagebox.askyesno(
                "Confirm Delete",
                f"Are you sure you want to delete Animation {group_idx + 1}?\n\nThis will remove all frames in this sequence.",
            ):
                del self.animation_group[group_idx]
                self.update_animation_group_listbox()
    ...
    def view_animation_sequences(self):
        if not self.animation_group:
            messagebox.showwarning(
                "No Animations",
                "There are no animation sequences to view.\n\nAdd some animations first.",
            )
            return
    ...
    def save_config(self):
        ...
        if file_path:
            ...
            messagebox.showinfo(
                "Configuration Saved", "Your configuration has been saved successfully."
            )
    ...
    def load_config(self):
        ...
        try:
            ...
            if not loaded_folder.exists():
                messagebox.showerror(
                    "Folder Not Found",
                    f"The folder specified in the configuration no longer exists:\n\n{loaded_folder}",
                )
                return
            ...
            def apply_config_values():
                ...
                result = self.validate_config_values(
                    ...
                    sprite_properties=config.get("sprite_properties"),
                )
                ...
                # Apply sprite_properties (each field individually so one wrong value doesn't reset others)
                props = valid.get("sprite_properties", {})
                if "sprite_category" in props:
                    self.sprite_category.set(props["sprite_category"])
                else:
    ...
    def validate_config_values(
        ...
        # ---- Validate sprite_properties ----
        if sprite_properties is not None:
            if not isinstance(sprite_properties, dict):
                invalid_values["sprite_properties"] = (
                    f"Must be a dictionary. Received: {type(sprite_properties).__name__}."
                )
            else:
                valid_props = {}
                invalid_prop_errors = []

                # Validate sprite_category (accepts display names)
                if "sprite_category" in sprite_properties:
                    cat = sprite_properties["sprite_category"]
                    if not isinstance(cat, str):
                        invalid_prop_errors.append(
                            f"'sprite_category': Must be a string. Received: {type(cat).__name__}."
                        )
                    elif cat not in CATEGORY_CHECKBOX_MAP:
                        invalid_prop_errors.append(
                            f"'sprite_category': Must be one of {list(CATEGORY_CHECKBOX_MAP.keys())}. Received: {cat}."
                        )
                    else:
                        valid_props["sprite_category"] = cat

                # Validate boolean properties
                for key in ["use_tiles_mode", "used_base_palette"]:
                    if key in sprite_properties:
                        val = sprite_properties[key]
                ...
                if valid_props:
                    valid_values["sprite_properties"] = valid_props
        ...
        def complete_validation():
            if sprite:
                if target == "main":
                    ...
                    if requires_base_sprite and self.fg_base_sprite is None:
                    ...
                    # Check if already-loaded base sprite is the correct type
                    elif requires_base_sprite and self.fg_base_sprite is not None:
                        base_type = self.fg_base_validation_info.get("base_type")
                        if base_type != requires_base_sprite:
                            ...
                            print(
                                f"[WARNING] Expected {expected_name} base sprite — loaded base is {loaded_name}."
                            )
                            if hint:
                                print(
                                    f"[HINT] The {expected_name} base is the {hint}.\n"
                                )
                    ...
                    # If main sprite is loaded, check compatibility
                    if self.fg_sprite:
                        requires_base_sprite = self.fg_validation_info.get(
                        ...
                        # No main sprite yet - just show info about what type of base was loaded
                        if base_type:
                            base_name, _ = BASE_SPRITE_INFO.get(
                                base_type, (base_type, "")
                            )
                            print(
                                f"[INFO] Loaded {base_name} base sprite. Select a main sprite to continue.\n"
                            )
                        else:
                            print("[WARNING] This file is not a valid base sprite.\n")
                            hint_lines = [
                                f"  • {name} base — {loc}"
