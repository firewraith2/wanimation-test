...
def reconstruct_frames(
    ...
    for frame_id, chunks_info in frames_dict.items():
        ...
        # Save all layers
        for layer_id, (layer_array, _, layer_palette_slot) in enumerate(layers_list):
            layer_img = Image.fromarray(layer_array)
            layer_img.putpalette(global_palette)
            out_path = output_folder / f"Frame-{frame_id + 1}-Layer-{layer_id + 1}.png"
...
def generate_frames_main(data):
    ...
    # Normalize avoid_overlap to internal format
    ...
    # Determine output folder based on input type
    if folder_or_wan_path.is_file():
        # For WAN files: create {wan_name}_frames in the same directory
        wan_name = folder_or_wan_path.stem
        output_folder = folder_or_wan_path.parent / f"{wan_name}_frames"
    else:
        # For folders: use {folder_name}_frames subfolder
        folder_name = folder_or_wan_path.name
        output_folder = folder_or_wan_path / f"{folder_name}_frames"

    output_folder.mkdir(parents=True, exist_ok=True)

    # Handle base file merging - only copy what sprite is missing
    if base_sprite and base_validation_info:
        is_animation_base = validation_info.get("is_animation_base", False)
        is_image_base = validation_info.get("is_image_base", False)
        base_is_image_base = base_validation_info.get("is_image_base", False)
        base_is_animation_base = base_validation_info.get("is_animation_base", False)

        if is_animation_base and base_is_image_base:
            # Input has animations, base has images - copy images from base
            print("[INFO] Using images/palette from image-only base sprite")
        ...
        elif is_image_base and base_is_animation_base:
            # Input has images, base has animations - copy animations from base
            print("[INFO] Using animations from animation-only base sprite")
    ...
    # Determine if base sprite is actually being used
    requires_base_sprite = validation_info.get("requires_base_sprite", False)
    uses_base_sprite = requires_base_sprite and base_sprite is not None

    # Get palette from sprite
    # Palette layout: with base=[base|unique], without base=[unique only]
    # Palette layout when NOT using base sprite: [unique_palette (slots 0+)]
    ...
    if uses_base_sprite and shared_palette is not None and len(shared_palette) > 0:
        ...
        if sprite.palette.size > 0:
            ...
            if unique_palette_end <= len(global_palette):
                ...
                # Extend global_palette if needed
        ...
        # No base sprite needed - use unique palette directly starting at slot 0
    ...
    # Save Tilemap
    if DEBUG:
        # Place DEBUG folder inside the output frames folder
...
def fg_process_single_folder(
    ...
    if validation_info["is_animation_base"] and base_sprite_path is None:
        print(
    ...
    if validation_info["is_image_base"] and base_sprite_path is None:
        print(
    ...
    if validation_info["requires_base_sprite"] and base_sprite_path is None:
        print(
    ...
    if base_sprite_path is not None:
        try:
            ...
            if validation_info["is_animation_base"] and not base_validation_info.get(
                "is_image_base"
            ):
                print(
                    "[WARNING] Animation-only input needs an image-only base sprite — provided file may not work correctly."
                )
            elif validation_info["is_image_base"] and not base_validation_info.get(
                "is_animation_base"
            ):
                print(
            ...
            ] and not base_validation_info.get("is_image_base"):
                print(
