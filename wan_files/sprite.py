    ...
    def validate(self, raise_on_errors: bool = True) -> dict:
        ...
        if DEBUG:
            if is_animation_base:
                print("[DEBUG] Detected animation-only base file (no images/palette)\n")
            elif is_image_base:
                print(
                    "[DEBUG] Detected image-only base file (no metaframes/animations)\n"
                )
        ...
        for mf in self.metaframes:
            if is_8bpp or mf.is_absolute_palette == 1:
                ...
                if palette_slot < palette_slot_base:
                    requires_base_sprite = True
                    break

        return {
            "is_image_base": is_image_base,
            "is_animation_base": is_animation_base,
