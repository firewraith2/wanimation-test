...
def run_tests(test_data_dir: Path, specific_files: list = None) -> dict:
    ...
    try:
        ...
        if specific_files:
            ...
            for f in missing:
                print(f"[FAIL] File not found: {test_data_dir / f}")
                results[f] = False
        else:
        ...
        if not wan_files:
            ...
            # Mark this as a test failure, not silent pass
            results["_no_files"] = False
            return results
        ...
        expected_folders = {f"{wf.stem}_extracted": wf for wf in wan_files}
        ...
        # Check for expected folders that weren't extracted
        extracted_names = {f.name for f in extracted_folders}
        for folder_name, wan_file in expected_folders.items():
            if folder_name not in extracted_names:
                print(
                    f"[FAIL] {wan_file.name} - Extraction failed (folder not created)"
                )
                results[wan_file.name] = False
        ...
        # Mark all pending WAN files as failed
        for wan_file in wan_files:
            if wan_file.name not in results:
                results[wan_file.name] = False
